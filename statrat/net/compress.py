...
class Compression:
    ...
    def decompress(self, data: bytes, *, override=False):
        ...
        if uncompressed_data_size == 0x00 or packet_size <= self.threshold:
            # Turns out `packet_size < self.threshold` is needed. I do not know why.
            # Possible header check error because wiki.vg says it has to be LARGER than the threshold... oops!
